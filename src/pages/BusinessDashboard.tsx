...
import { BarChart, Settings, Image, Clock, Menu as MenuIcon, Star, Eye } from 'lucide-react';
...
export default function BusinessDashboard() {
  const [activeTab, setActiveTab] = useState<'overview' | 'profile' | 'menu' | 'hours'>('overview');
  const { user } = useAuth();
  const { stalls, getReviewsByStall } = useData();

  // Mock: Get the business's stall (in real app, filter by user ID)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-lg shadow-sm">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                  ...
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    <div className={`w-8 h-8 rounded-full mx-auto mb-2 ${
                      businessStall.isOpen ? 'bg-green-500' : 'bg-red-500'
                    }`}></div>
                    <div className="text-lg font-bold text-gray-900">
                      {businessStall.isOpen ? 'Open' : 'Closed'}
                    </div>
                ...
                <div className="border-t pt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <img
                        src={businessStall.images[0]}
                        alt={businessStall.name}
                    ...
                    <div className="space-y-3">
                      ...
                      <p><strong>Price Range:</strong> {businessStall.priceRange}</p>
        ...
        {activeTab === 'profile' && (
          <StallProfileEditor stall={businessStall} />
        )}

        {activeTab === 'menu' && (
          <MenuEditor stall={businessStall} />
        )}

        {activeTab === 'hours' && (
          <HoursEditor stall={businessStall} />
        )}
      </div>
    </div>
  );
