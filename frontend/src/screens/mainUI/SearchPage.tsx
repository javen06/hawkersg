...
export default function SearchPage() {
  ...
  const filteredStalls = validStalls.filter((stall) => {
    ...
    const normalizedRating = Math.min(Math.max(stall.rating ?? 0, 2.0), 5.0);
    const matchesRating = !filters.rating || normalizedRating >= filters.rating - 0.05;
    const matchesOpen = !filters.isOpen || stall.isOpen;

    return (
      matchesQuery && matchesCuisine && matchesPrice && matchesRating && matchesOpen
    );
  });
